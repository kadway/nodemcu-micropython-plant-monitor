...
class Stm32Spi:
    ...
    def get_data(self, max_elements=10):
        ...
        # empty the dictionary before adding more data to avoid memory problems
    ...
    def unpack_general_conf(self):
        ...
        self.default_commands[b'get_general_config'][2]['adc_interval'] = self.time_temp[0] | self.time_temp[1] << 16
        self.default_commands[b'get_general_config'][2]['init_code'] = unpack('<H', self.byteArr[4:6])[0]
        self.default_commands[b'get_general_config'][2]['page_adc'] = unpack('<H', self.byteArr[6:8])[0]
        self.default_commands[b'get_general_config'][2]['page_act'] = unpack('<H', self.byteArr[8:10])[0]
        self.default_commands[b'get_general_config'][2]['page_offset_adc'] = unpack('<H', self.byteArr[10:12])[0]
        self.default_commands[b'get_general_config'][2]['page_offset_act'] = unpack('<H', self.byteArr[12:14])[0]
        self.default_commands[b'get_general_config'][2]['num_areas'] = int(self.byteArr[14])
        self.default_commands[b'get_general_config'][2]['num_sensors'] = int(self.byteArr[15])
        self.default_commands[b'get_general_config'][2]['num_pumps'] = int(self.byteArr[16])
        self.default_commands[b'get_general_config'][2]['num_sovs'] = int(self.byteArr[17])
        # last 2 bytes are not relevant, exist because of padding in stm32 data structure

    def unpack_area_conf(self, idx):
        self.default_commands[b'get_area_config'][2]['area' + str(idx)] = {}
        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['associated_sensors'] = [0 for i in range(15)]
        for i in range(15):
            self.default_commands[b'get_area_config'][2]['area' + str(idx)]['associated_sensors'][i] = int(self.byteArr[i])

        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['associated_solenoids'] = [0 for i in range(10)]
        for i in range(10):
            self.default_commands[b'get_area_config'][2]['area' + str(idx)]['associated_solenoids'][i] = int(self.byteArr[15 + i])

        self.time_temp = unpack('<HH', self.byteArr[28:32])
        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['watering_duration'] = self.time_temp[0] | self.time_temp[1] << 16

        self.time_temp = unpack('<HH', self.byteArr[32:36])
        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['watering_interval'] = self.time_temp[0] | self.time_temp[1] << 16

        self.time_temp = unpack('<HH', self.byteArr[36:40])
        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['last_watering_time'] = self.time_temp[0] | self.time_temp[1] << 16

        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['threshold'] = unpack('<H', self.byteArr[40:42])[0]

        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['associated_pump'] = int(self.byteArr[42])

        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['open_loop'] = "yes" if int(self.byteArr[43]) == 1 else "no"

        self.default_commands[b'get_area_config'][2]['area' + str(idx)]['area_ID'] = int(self.byteArr[44])
